...
`SWORDSTAMP_TEST_EMBEDDER` to a supplied local watermark-encoder checkpoint.
...
uv run python -m unittest -v tests.test_pmark_bridge tests.test_samark_bridge
```
...
| Release metadata, dependencies, and setup | `test_release_metadata.py` |
| Bundled scheduler safety and CPU dependency DAG | `test_bundled_scheduler.py` |
| Public PMark/SAMark I/O and calibration bridges | `test_pmark_bridge.py`, `test_samark_bridge.py` |
