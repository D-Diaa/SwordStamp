...
_CALIBRATION_PATH = os.path.join(_ROOT, "external", "pmark", "calibration.py")
...
class PMarkBridgeTests(unittest.TestCase):
    ...
    def test_sampler_and_detector_settings_are_wired(self):
        with open(os.path.join(_ROOT, "external", "pmark", "utils", "detect.py")) as f:
            detector = f.read()
    ...
    def test_hf_exports_preserve_early_stopped_pmark_and_samark_rows(self):
        with open(os.path.join(_ROOT, "external", "pmark", "pmark.py")) as f:
            pmark = f.read()
        ...
        for name in ("samark_gen.py", "samark_gen_unwatermarked.py"):
            with self.subTest(generator=name):
                with open(os.path.join(_ROOT, "external", "SAMark", name)) as f:
                    samark = f.read()
