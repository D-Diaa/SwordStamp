...
without 3%-tolerance bigram selection; DIPPER at $(L,O)=(20,20),(20,40),
(20,80),(40,20),(80,20)$; the 16 controlled reorder, synonym, split, and merge
...
The comparison runners each expose one paper cell:

- `pmark.sh`: online/HD PMark, rejection, sentence-NLTK, N=64, signature width
  4, soft count K=150 and delta 0.001;
- `samark.sh`: one flag pattern per run, rejection, sentence-NLTK, N=64,
  signature width 2, and the paper's fixed transform and thresholds.
...
PMark runs from `external/pmark`; SAMark runs from `external/SAMark`. Install
their documented environments after initializing the submodules.
...
- `_pmark_*` and `_samark_*` bridge the comparison submodules.
