...
def generate_next_sentences_hf(
    model, tokenizer, prompt, num_sentences, debug=False, dedup=True,
...
def sample_next_sentence_msignal(
    ...
    for _ in range(5):
        if llm:
            gen_texts = generate_next_sentences_vllm(
                prompt=prompt, llm=llm, num_samples=num_samples, dedup=dedup,
                temperature=temperature, top_p=top_p,
                repetition_penalty=repetition_penalty,
            )
        elif openai_api_base:
            gen_texts = generate_next_sentences_api(
                prompt=prompt, num_samples=num_samples, model_name=model_name,
                openai_api_base=openai_api_base, dedup=dedup,
                temperature=temperature, top_p=top_p,
            )
        else:
            gen_texts = generate_next_sentences_hf(
                model, tokenizer, prompt, num_samples, dedup=dedup,
                temperature=temperature, top_p=top_p,
                repetition_penalty=repetition_penalty,
            )
        end = time.time()
