...
def within_green(
    curr_sen, prompt, rand_flag, llm, embedder, num_samples,
    api_base=None, model_name=None, pivot="rand", median_method="torch",
    debug=False, dedup=False, temperature=0.9, top_p=0.9,
    repetition_penalty=1.05,
):
    for _ in range(3):
        sampled_texts = generate_next_sentences_vllm(
            prompt=prompt, llm=llm, num_samples=num_samples, dedup=dedup,
...
def within_green_msig(
    curr_sen, prompt, rand_flags, llm, embedder, num_samples,
    median_method="torch", K=150, threshold=0.001, debug=False,
    dedup=False, min_seqs=10, model=None, tokenizer=None,
    temperature=0.9, top_p=0.9, repetition_penalty=1.05,
):
    if median_method!="prior":
        for _ in range(3):
            if model:
                sampled_texts = generate_next_sentences_hf(
                    prompt=prompt, model=model, tokenizer=tokenizer,
                    num_sentences=num_samples, dedup=dedup,
                    temperature=temperature, top_p=top_p,
                    repetition_penalty=repetition_penalty,
                )
            else:
...
def watermark_z_test(within_greens, alpha=0.05, debug=False):
    """
    One-sided z-test for watermark detection (theoretical null: p0 = 0.5).

    Returns plain Python floats/bool so callers don't need .item().
    Returns (False, 0.0, 1.0) when there are fewer than 2 sentences.
    """
    n = len(within_greens)
    if n < 2:
    ...
    z_score = float((p_hat - p0) / standard_error)
    p_value = float(1 - stats.norm.cdf(z_score))
    is_watermarked = bool(p_value < alpha)
    return is_watermarked, z_score, p_value


def detect_paragraph(
    sentences, num_samples=None, api_base=None, model_name=None, llm=None,
    embedder=None, pivot=None, median_method=None, debug=False, dedup=False,
    msig=False, model=None, tokenizer=None, soft_k=150, soft_delta=0.001,
    temperature=0.9, top_p=0.9, repetition_penalty=1.05,
):
    """
    ...
    for i in range(1, len(sentences)):
        ...
        if msig<=1:
            rand_flag=secret_sbit[i % len(secret_sbit)]
            res = within_green(
                curr_sen=sentences[i], prompt=prompt, rand_flag=rand_flag,
                llm=llm, embedder=embedder, num_samples=num_samples,
                api_base=api_base, model_name=model_name, pivot=pivot,
                median_method=median_method, debug=False, dedup=dedup,
                temperature=temperature, top_p=top_p,
                repetition_penalty=repetition_penalty,
            )
        else:
            rand_flags=secret_mbit[i % len(secret_mbit)]
            res = within_green_msig(
                curr_sen=sentences[i], prompt=prompt, rand_flags=rand_flags,
                llm=llm, embedder=embedder, num_samples=num_samples,
                median_method=median_method, K=soft_k, threshold=soft_delta,
                debug=False, dedup=dedup, model=model, tokenizer=tokenizer,
                temperature=temperature, top_p=top_p,
                repetition_penalty=repetition_penalty,
            )
