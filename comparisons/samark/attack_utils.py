...
class ParrotParaphraseConfig:
    ...
    use_bigram_filter: bool = True
    bert_threshold: float = 0.03
    bigram_tokenizer_name: str="facebook/opt-1.3b"
...
class GPTTranslator:
    """Translate text using GPT API (back-translation via GPT)."""
    def __init__(self, source_lang, target_lang, openai_model='gpt-3.5-turbo'):
        self.source_lang = source_lang
        self.target_lang = target_lang
        self.openai_model = openai_model
        self.client = _make_openai_client()
        # Optional alternative provider can be configured via OPENAI_BASE_URL.

    @backoff.on_exception(backoff.expo, openai.RateLimitError)
    def translate(self, text):
        prompt = f"Translate the following text from {self.source_lang} to {self.target_lang}. Only output the translation, nothing else:\n\n{text}"
        while True:
            try:
                response = self.client.chat.completions.create(
                    model=self.openai_model,
                    messages=[
                        {"role": "system", "content": f"You are a professional translator. Translate text from {self.source_lang} to {self.target_lang} accurately."},
...
class KimiTranslator:
    """Translate text using Kimi API (back-translation via kimi-k2.5)."""
    def __init__(self, source_lang, target_lang, openai_model='kimi-k2.5'):
        self.source_lang = source_lang
        self.target_lang = target_lang
        self.openai_model = openai_model
        self.client = _make_openai_client()
...
def get_attacker(attack_name,ratio,device="cuda"):
    if attack_name == 'Word-D':
      ...
      attack = ContextAwareSynonymSubstitution(ratio=ratio,
        ...
        shared_llm = LLM(model="../models/Meta-Llama-3-8B",
    ...
    elif attack_name == 'Doc-T(GPT)':
        en2zh = GPTTranslator("English", "Chinese", openai_model='gpt-3.5-turbo')
        zh2en = GPTTranslator("Chinese", "English", openai_model='gpt-3.5-turbo')
        attack = BackTranslationTextEditor(en2zh.translate, zh2en.translate)
    elif attack_name == 'Doc-T(GPT-ES)':
        en2es = GPTTranslator("English", "Spanish", openai_model='gpt-3.5-turbo')
        es2en = GPTTranslator("Spanish", "English", openai_model='gpt-3.5-turbo')
        attack = BackTranslationTextEditor(en2es.translate, es2en.translate)
    elif attack_name == 'Doc-P I':
        attack = GPTParaphraser(openai_model='gpt-3.5-turbo',
                                prompt='Please rewrite the following text: ')
    elif attack_name == 'Doc-P II':
        attack = GPTParaphraser(openai_model='gpt-3.5-turbo',
                                prompt='Please rewrite the following text, avoiding using the same words or phrases in original text: ')
