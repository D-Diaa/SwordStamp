...
class OpenAIAPI:
    ...
    def __init__(self, model, temperature, system_content):
        ...
        self.client = _get_api_client()
        # List of supported models
        supported_models = ['gpt-3.5-turbo', 'gpt-4', 'kimi-k2.5']
    ...
    def get_result_from_kimi(self, query):
        """get result from Kimi k2.5 model."""
        response = self.client.chat.completions.create(
            model="kimi-k2.5",
            temperature=self.temperature,
            messages=[
        ...
        while True:
            try:
                if self.model == 'gpt-3.5-turbo':
                    ...
                    result = self.get_result_from_kimi(query)
                break
...
if __name__ == "__main__":
    openai_util = OpenAIAPI('gpt-4', 0.2, "Your are a helpful assistant to rewrite the text.")
